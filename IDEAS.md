...
| [6.19](docs/agent-log.md#619-the-loop-run-twice--shipped-verb-landed-in-620) | The loop, run twice | shipped (verb landed in §6.20) |
...
| [6.28](docs/agent-log.md#628-concurrent-writers-the-known-limit-quantified--measured-fix-deferred-to-store-replacement) | Concurrent writers: the known limit, quantified | measured, fix deferred to store replacement |
...
| [6.31](docs/agent-log.md#631-nothing-that-persists-carries-a-version--measured-store-ratified-decision-0121-ledger-still-open) | Nothing that persists carries a version | measured, store ratified (decision 0121), ledger still open |
...
| [6.33](docs/agent-log.md#633-the-memory-has-never-been-given-the-projects-decisions--measured-corpus-gap-closed-matcher-limitation-tracked-by-6946106) | The memory has never been given the project's decisions | measured, corpus gap closed; matcher limitation tracked by §6.94/§6.106 |
...
| [6.35](docs/agent-log.md#635-the-solo-verifier-two-records-kept-carefully-and-shown-to-nobody--measured-fix-blocked-on-design-decision) | The solo verifier: two records kept carefully and shown to nobody | measured, fix blocked on design decision |
...
| [6.37](docs/agent-log.md#637-the-entity-graph-destroys-what-the-numeric-recipe-keeps-and-has-no-word-for-an-ambiguous-name--measured-fix-blocked-on-622-design) | The entity graph destroys what the numeric recipe keeps, and has no word for an ambiguous name | measured, fix blocked on §6.22 design |
| [6.38](docs/agent-log.md#638-locks_in_text-is-a-raw-substring-so-a-short-lock-fires-inside-longer-words--measured-fix-shipped) | `locks_in_text` is a raw substring, so a short lock fires inside longer words | measured, fix shipped |
| [6.39](docs/agent-log.md#639-the-entity-graph-has-only-the-verb-a-machine-may-not-use--measured-fix-shipped) | The entity graph has only the verb a machine may not use | measured, fix shipped |
...
| [6.42](docs/agent-log.md#642-the-quorum-memos-step-2-has-been-unrunnable-and-its-zero-would-have-been-unreadable--measured-blocked-on-deployment-chain) | The quorum memo's step 2 has been unrunnable, and its zero would have been unreadable | measured, blocked on deployment chain |
| [6.43](docs/agent-log.md#643-dogfood_storepy---verify-says-the-store-matches-the-decision-files-and-does-not-check-where-a-row-came-from--measured-fix-shipped) | `dogfood_store.py --verify` says the store matches the decision files, and does not check where a row came from | measured, fix shipped |
| [6.44](docs/agent-log.md#644-nestor_propose-discards-a-forbidden-argument-without-saying-so--measured-fix-shipped) | `nestor_propose` discards a forbidden argument without saying so | measured, fix shipped |
...
| [6.49](docs/agent-log.md#649-the-staleness-memos-2-names-the-wrong-timestamp-as-unmovable-by-one-entry--measured-listing-shipped-caveat-addressed-api-wired) | The staleness memo's §2 names the wrong timestamp as unmovable, by one entry | measured, listing shipped, caveat addressed (API wired) |
...
| [6.53](docs/agent-log.md#653-origin-now-says-what-produced-the-row-which-forced-the-extractors-into-the-repository--shipped-visibility-field-blocked-on-schema-migration-decision) | `origin` now says what produced the row, which forced the extractors into the repository | shipped, visibility field blocked on schema migration decision |
...
| [6.94](docs/agent-log.md#694-the-decision-store-answers-its-own-questions-well-except-where-its-matcher-cannot-tell-two-decisions-apart--measured-partially-shipped-constraints_on-fuzzy-at-055-serve-path-bar-still-at-092) | The decision store answers its own questions well, except where its matcher cannot tell two decisions apart | measured, partially shipped (`constraints_on` fuzzy at 0.55; serve-path bar still at 0.92) |
...
| [6.97](docs/agent-log.md#697-detailpanel-renders-two-literal-null-text-nodes-into-the-provenance-card--verified-fix-shipped) | `detailPanel` renders two literal `null` text nodes into the provenance card | verified, fix shipped |
| [6.98](docs/agent-log.md#698-bench-and-scriptsaudit_py-inherit-the-ambient-keyring-and-report-a-false-fails--measured-fix-shipped) | `bench/` and `scripts/audit_*.py` inherit the ambient keyring, and report a false `FAILS` | measured, fix shipped |
| [6.99](docs/agent-log.md#699-an-llm-standing-in-for-the-embedder-is-self-consistent-inside-a-conversation-and-drifts-between-them--measured-conclusion-recorded) | An LLM standing in for the embedder is self-consistent inside a conversation and drifts between them | measured, conclusion recorded |
| [6.100](docs/agent-log.md#6100-one-gate-for-every-change-class-and-what-that-costs-a-session-with-a-human-waiting-in-it--measured-fix-shipped) | One gate for every change class, and what that costs a session with a human waiting in it | measured, fix shipped |
| [6.101](docs/agent-log.md#6101-the-corpus-extractors-do-not-fail-closed-and-the-test-named-for-them-covers-a-different-family--verified-fix-shipped) | The corpus extractors do not fail closed, and the test named for them covers a different family | verified, fix shipped |
| [6.102](docs/agent-log.md#6102-the-extractors-walk-the-working-tree-so-following-this-repos-own-setup-instructions-poisons-its-corpus--verified-fix-shipped) | The extractors walk the working tree, so following this repo's own setup instructions poisons its corpus | verified, fix shipped |
| [6.103](docs/agent-log.md#6103-a-model-survey-of-vendors-got-two-licences-exactly-backwards-in-the-same-row--verified-lesson-shipped) | A model survey of vendors got two licences exactly backwards, in the same row | verified, lesson shipped |
| [6.104](docs/agent-log.md#6104-the-jeles-source-registrys-gaps-live-in-a-feeders-stdout-and-i-misreported-one-by-reading-the-top-of-it--measured-fix-shipped) | The jeles source registry's gaps live in a feeder's stdout, and I misreported one by reading the top of it | measured, fix shipped |
| [6.105](docs/agent-log.md#6105-the-fleets-own-decision-record-is-invisible-to-every-corpus-extractor--verified-fix-shipped) | The fleet's own decision record is invisible to every corpus extractor | verified, fix shipped |
| [6.106](docs/agent-log.md#6106-where-the-decision-stores-retrieval-actually-fails-rank-is-fine-for-content-bearing-questions-and-collapses-for-question-shaped-ones--measured-partially-shipped-same-fix-as-694) | Where the decision store's retrieval actually fails: rank is fine for content-bearing questions and collapses for question-shaped ones | measured, partially shipped (same fix as §6.94) |
| [6.107](docs/agent-log.md#6107-the-ui-was-built-for-an-operator-who-read-the-docstrings-the-audience-it-is-about-to-meet-has-not--shipped) | The UI was built for an operator who read the docstrings; the audience it is about to meet has not | shipped |
| [6.108](docs/agent-log.md#6108-the-web-sessionstart-hook-depends-on-an-env-var-the-runtime-did-not-set-fails-silently-and-the-seat-policy-that-would-have-caught-the-miss-fails-through-the-same-door--verified-fix-shipped-residual-upstream-not-fixable-from-this-tree) | The web SessionStart hook depends on an env var the runtime did not set, fails silently, and the seat policy that would have caught the miss fails through the same door | verified, fix shipped (residual: upstream, not fixable from this tree) |
...
| [6.127](docs/agent-log.md#6127-a-matcher-change-orphaned-every-stored-key-with-a-hyphen-and-the-seal-endpoint-reported-success-for-doing-nothing--measured-listing-fix-shipped-cause-and-contract-open) | A matcher change orphaned every stored key with a hyphen, and the seal endpoint reported success for doing nothing | measured; listing fix shipped, cause and contract open |
| [6.128](docs/agent-log.md#6128-standing-up-a-household-made-a-green-tree-red-and-the-survey-caught-three-repos-it-had-silently-dropped--measured-both-fixes-shipped-the-ambient-read-pattern-open) | Standing up a household made a green tree red, and the survey caught three repos it had silently dropped | measured; both fixes shipped, the ambient-read pattern open |
