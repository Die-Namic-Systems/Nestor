...
| [6.108](docs/agent-log.md#6108-the-web-sessionstart-hook-depends-on-an-env-var-the-runtime-did-not-set-fails-silently-and-the-seat-policy-that-would-have-caught-the-miss-fails-through-the-same-door--verified-fix-open) | The web SessionStart hook depends on an env var the runtime did not set, fails silently, and the seat policy that would have caught the miss fails through the same door | verified, fix open |
