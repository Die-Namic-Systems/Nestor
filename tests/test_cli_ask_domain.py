...
configured domain (decision → decision) wins when the store actually has rows in it,
...
def test_a_store_holding_the_configured_domain_keeps_it(db):
    memory.add_pair("may we ship?", "yes", "decision", "decision",
                    status="sealed", verifier="rita", store=db["store"])
    # Even with a larger competing domain present, decision→decision wins because
    # the store actually holds it — askDomain() checks membership, not size, first.
    for i in range(5):
        memory.add_pair(f"Good evening {i}.", f"Buenas noches {i}.", "en", "es",
                        status="sealed", verifier="rita", store=db["store"])
    assert cli._ask_domain(db["store"], None, None) == ("decision", "decision")
...
def test_an_explicit_flag_is_used_as_is_even_if_the_store_lacks_it(db):
    ...
    assert cli._ask_domain(db["store"], "fr", None) == ("fr", "decision")
    assert cli._ask_domain(db["store"], None, "de") == ("decision", "de")
...
def test_nestor_ask_with_no_flags_and_no_matching_domain_stays_pending(db, capsys):
    # Nothing sealed anywhere: the configured decision→decision default holds,
    # and the phrase is correctly reported unanswered rather than crashing.
